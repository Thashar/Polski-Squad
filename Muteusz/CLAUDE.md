...
14. **App Sync Backfill** - `interactionHandlers.handleAppSyncBackfillCommand`: Komenda `/appsync-backfill` (admin-only) uruchamia `AppBackfillRunner` (`utils/appBackfill.js`), który wypycha historyczne dane z lokalnych JSON-ów (Stalker/data, shared_data) do web API przez batchowe endpointy `/api/bot/<resource>/batch` (500 items/req, raw SQL `ON CONFLICT DO UPDATE` lub `createMany skipDuplicates` po stronie API).
   - Opcje: `resource` (jeden z 8), `bot` (stalker/kontroler/endersecho), `guild` (filtr guildId), `api_url` (opcjonalny string — nadpisuje URL docelowy, np. dla pushu na staging), `dry_run` (tylko zlicza bez pushowania)
   ...
   - Embed: progress bar per zasób (▓▓▓░░░░░░░), postęp całkowity, filtry (z targetem + URL), czas trwania, kolor zielony/czerwony/pomarańczowy w zależności od stanu
   - **URL routing:** domyślnie leci na produkcję (`APP_API_URL` z env). Parametr `api_url` nadpisuje TYLKO URL — token zawsze z env (`BOT_API_KEY`). Sync endpointy (`/api/bot/<resource>/batch`) weryfikują tylko valid bot token, bot identyfikowany jest przez payload (`guildId`, `discordId`) — per-bot klucze są potrzebne TYLKO dla operation gateway, nie dla backfillu. Runner buduje własny klient przez `createAppSync({ apiUrl, apiKey })` z `utils/appSync.js`, dzięki czemu custom-push nie wpływa na regularny ruch API z pozostałych botów. Brak `APP_API_URL` lub `BOT_API_KEY` → ostrzeżenie przed startem.
