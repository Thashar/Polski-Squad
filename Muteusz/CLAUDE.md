...
14. **App Sync Backfill** - `interactionHandlers.handleAppSyncBackfillCommand`: Komenda `/appsync-backfill` (admin-only) uruchamia `AppBackfillRunner` (`utils/appBackfill.js`), który wypycha historyczne dane z lokalnych JSON-ów (Stalker/data, shared_data) do web API przez batchowe endpointy `/api/bot/<resource>/batch` (500 items/req, raw SQL `ON CONFLICT DO UPDATE` lub `createMany skipDuplicates` po stronie API).
   - Opcje: `resource` (jeden z 8), `bot` (stalker/kontroler/endersecho), `guild` (filtr guildId), `target` (production/staging, domyślnie production), `dry_run` (tylko zlicza bez pushowania)
   ...
   - **Target routing:** domyślny `production` używa `APP_API_URL` + `BOT_API_KEY` (tych samych co hot-path sync z botów). `staging` wymaga osobnego kompletu `APP_API_URL_STAGING` + `BOT_API_KEY_STAGING` — runner buduje własny klient przez `createAppSync()` z `utils/appSync.js`, dzięki czemu staging-push nie wpływa na regularny ruch API z pozostałych botów. Brak env dla wybranego targetu → ostrzeżenie przed startem.
