...
14. **App Sync Backfill** - `interactionHandlers.handleAppSyncBackfillCommand`: Komenda `/appsync-backfill` (admin-only) uruchamia `AppBackfillRunner` (`utils/appBackfill.js`), który wypycha historyczne dane z lokalnych JSON-ów (Stalker/data, shared_data) do web API przez batchowe endpointy `/api/bot/<resource>/batch` (500 items/req, raw SQL `ON CONFLICT DO UPDATE` lub `createMany skipDuplicates` po stronie API).
   ...
   - **URL routing:** domyślnie leci na produkcję (`APP_API_URL` z env). Parametr `api_url` nadpisuje TYLKO URL — token `BOT_API_KEY` zawsze z env (ten sam dla staging/produkcji, więc osobnych env-ów nie potrzebujemy). Runner buduje własny klient przez `createAppSync()` z `utils/appSync.js`, dzięki czemu custom-push nie wpływa na regularny ruch API z pozostałych botów. Brak `APP_API_URL` w env i brak parametru `api_url` → ostrzeżenie przed startem.
