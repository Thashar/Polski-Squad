...
16. **Komenda /clean — czyszczenie kanału BEZ limitu ilości** - `interactionHandlers.js` → `handleCleanCommand` + `collectMessagesToClean` / `deleteCollectedMessages` / `logCleanedMessages`. Uprawnienie `ModerateMembers`, odpowiedź ephemeral + publiczne potwierdzenie.
   - **Skąd brał się dawny limit 100:** komenda robiła JEDNO `messages.fetch({ limit: 100 })` i jeden `bulkDelete` — a 100 to limit Discord API na pojedyncze żądanie, nie limit możliwości. Teraz pobieranie **stronicuje** kursorem `before`, a kasowanie leci **porcjami po 100**, więc liczbę wiadomości ogranicza wyłącznie zawartość kanału. Zniknęły też `setMaxValue(100)` w opcji slash oraz `maxMessages`/`maxMinutes` w configu (`null` = bez limitu)
   - **⚠️ Granicy 14 dni obejść SIĘ NIE DA** — `bulkDelete` odrzuca starsze wiadomości po stronie Discorda (błąd 50034). Takie wiadomości lecą pojedynczym `msg.delete()` z przerwą `oldMessageDelayMs` (1100 ms), żeby nie wpaść w rate limit kanału. Jest to z natury wolne: 100 starych wiadomości to ~2 minuty. Liczba usuniętych tą drogą trafia do logu jako `🕰️ Starsze niż 14 dni`
   - **`scanLimit: 50000` to bezpiecznik, NIE limit kasowania** — chroni przed przeczesywaniem całego kanału, gdy filtr użytkownika prawie nic nie łapie. Po jego osiągnięciu komenda mówi wprost, ile przeskanowała i że trzeba ją powtórzyć; nic nie jest ucinane po cichu
   - **Postęp** odświeżany przez `editReply` nie częściej niż co 2 s (faza przeszukiwania i faza kasowania). ⚠️ Token interakcji żyje 15 minut — bardzo długie czyszczenie może stracić możliwość edycji odpowiedzi, ale samo kasowanie i log lecą dalej
   - **Czas nie jest już limitowany limitem mute.** `parseTimeFormat(timeString, maxMinutesOverride)` przyjmuje własny limit; `/clean` podaje `null` (bez limitu), `/mute` bez drugiego argumentu zachowuje swoje 7 dni. Wcześniej zakres `/clean` ograniczały DWIE niezależne bramki: twarde 1000 minut w handlerze i limit mute w parserze
17. **Kompaktowy log czyszczenia** - `logCleanedMessages` wysyła **jeden embed na całą operację** na kanał `clean.logChannelId` (brak = ten sam co `deletedMessageLogs.logChannelId`): kanał, kto wykonał, ile usunięto, użyty filtr oraz podgląd treści w bloku kodu (`[DD.MM GG:MM] Autor: treść`, od najstarszej).
   - **Dlaczego osobny format, a nie `mediaService.handleDeletedMessage`:** tamten wysyła **embed na każdą wiadomość**. Przy `/clean` kasującym setki pozycji byłoby to setki wiadomości i natychmiastowy rate limit — dlatego log jest zbiorczy
   - **Komplet treści idzie w załączniku `.txt`**, gdy nie mieści się w podglądzie (limit 4096 znaków opisu embeda). Dzięki temu nic nie jest ucinane po cichu; przy paru wiadomościach załącznika nie ma, bo byłby tylko hałasem
   - **⚠️ Bez odsiewania log by się DUBLOWAŁ.** Kasowanie pojedyncze (wiadomości >14 dni) wywołuje event `MessageDelete`, a ten wysyła embed na każdą wiadomość. Dlatego `/clean` zgłasza ID przez `mediaService.markMessagesAsCleaned()`, a `handleDeletedMessage` je pomija (wpisy wygasają po 10 min). Kasowanie masowe emituje `MessageDeleteBulk`, którego nikt nie nasłuchuje, więc tamtej ścieżki problem nie dotyczy
   - `mediaService` wstrzykiwany do handlera **setterem** `setMediaService()`, nie kolejnym parametrem pozycyjnym — konstruktor `InteractionHandler` ma ich już sześć
...
**Komendy:** `/clean`, `/io`, `/remove-roles`, `/special-roles`, `/add-special-role`, `/remove-special-role`, `/list-special-roles`, `/violations`, `/unregister-command`, `/chaos-mode`, `/msg`, `/data-archive`, `/restore-backup`, `/zgłoś`, context: `Zgłoś wiadomość`, `Wycisz użytkownika`, `Ostrzeż użytkownika`
...
- **Persistencja przez `utils/jsonStore` (cache-first):** wszystkie pliki `data/` — ostrzeżenia, wyzwiska, aktywne wyciszenia, liczniki naruszeń, blokady słów i obrazów, chaos mode, role specjalne, konflikty ról, cache członków, statystyki zgłoszeń, timery reakcji-ról oraz dane Rekrutera czytane przez `roleKickingService`. Odczyt z dysku raz, przy pierwszym sięgnięciu; zapis atomowy (plik tymczasowy + rename)
  - **⚠️ Żadnego `fs.existsSync` przed `store.getSync`** — store sam oddaje wartość domyślną przy braku pliku. Taki strażnik nie tylko dublował odczyt z dysku, ale potrafił ZGUBIĆ dane: `setSync` zapisuje plik w tle i atomowo (plik tymczasowy + rename), więc tuż po zapisie plik docelowy przez moment nie istnieje. `loadActiveMutes()` trafiając w to okno (np. w pętli `removeMute` w `initializePersistentMutes`) zwracało pustą listę, a następny `saveActiveMutes()` kasował WSZYSTKIE pozostałe wyciszenia razem z ich czasem odmute — czyli wyciszenie na stałe
