...
6. **Loteria Glory** - `gloryLotteryService.js`: cotygodniowe losowanie rangi Glory Member na podstawie progresu Fazy 1 (dane od Stalkera), **niezależne od `lotteryService`**
   ...
   - **Losowanie:** cron **piątek 22:00** czasu polskiego (`utils/timezone.js`, setTimeout jak MVP), osobne dla każdego klanu; pula ważona, **3 zwycięzców/klan** (`config.glory.winnersCount`), bez powtórzeń. Ogłoszenie embedem na kanale klanu (env `KONTROLER_GLORY_CHANNEL_*`) z **pingiem roli klanowej**. Embed zwycięzców pokazuje też **standard tygodnia** (średni progres) i konkretne progi losów dla danego klanu
