...
5. **MVP tygodnia** - `mvpService.js`:
   ...
   - **Aprobata MVP (reakcja KEKW aktualnego MVP):** Gdy posiadacz roli MVP tygodnia (`roleId`) zostawi reakcję KEKW pod **cudzym** postem, bot odpala LOSOWY „stempel aprobaty". Niezależne od ankiety tygodniowej.
     - **Losowanie efektu:** pojedynczy los z progami skumulowanymi wg stałych szans: jackpot (`jackpotChance` ~1%) → `textreply` (`textReplyChance` ~9%, „znak jakości", losowy tekst z puli) → `crown` (`crownChance` ~30%) → `stamp` (~60%, reszta — zawsze domyka do 100%)
     - **Efekty:** `stamp` (bot dorzuca pod postem reakcje-pieczęcie, domyślnie 🏅⭐💯🔥👏🏆 z `stampEmojis`), `crown` (autor dostaje prefix 👑 w nicku na 1h przez współdzielony `NicknameManager`), `embed` (ozdobny embed gratulacyjny — **wyłącznie** w jackpocie)
