...
5. **MVP tygodnia** - `mvpService.js`:
   ...
   - **Aprobata MVP (reakcja KEKW aktualnego MVP):** Gdy posiadacz roli MVP tygodnia (`roleId`) zostawi reakcję KEKW pod **cudzym** postem, bot odpala LOSOWY „stempel aprobaty". Niezależne od ankiety tygodniowej.
     - **Losowanie efektu:** pojedynczy los z progami skumulowanymi wg stałych szans: jackpot (`jackpotChance` 1%) → `textreply` (`textReplyChance` 9%, „znak jakości", losowy tekst z puli) → `stamp` (`stampChance` 15%) → `crown` (`crownChance` 25%) → **`none` (reszta, 50% – nic się nie dzieje)**. Pusty los też zużywa dedup posta (`effect: "none"` w `mvp_approvals.json`), więc ponowna reakcja pod tym samym postem nie daje drugiej szansy
   ...
   - **Konfiguracja:** `config.mvp` (pollChannelId, roleId, kekwEmojiId, voteEmojis, scanDays, targetAuthors, maxCandidates, votingDurationMs, scheduleWeekday/Hour/Minute, excludedChannels, **approval**: enabled, crownDurationMs, crownPrefix, jackpotChance, wildcardOnJackpot, textReplyChance, stampChance, crownChance, stampEmojis, maxApprovedMemory)
6. **Loteria Glory** - `gloryLotteryService.js`: cotygodniowe losowanie rangi Glory Member na podstawie progresu Fazy 1 (dane od Stalkera), **niezależne od `lotteryService`**
   ...
   - **`/glory-reroll`** (administrator LUB lider klanu): bez opcji – po wywołaniu pokazuje ephemeral **listę klanów do wyboru** (`StringSelectMenu` `glory_reroll_select`), a po wyborze dobiera dodatkowego zwycięzcę spośród uczestników ostatniego losowania, którzy nie wygrali (system awaryjny). **Kto co widzi (`getGloryRerollAllowedClans`):** administrator → wszystkie 4 klany; posiadacz roli z `config.glory.leaderRoles` (env `LEADER_ROLE` – wspólna rola Lidera akademii, `STALKER_LME_ADMIN_ROLE` – Lider klanu głównego) → tylko klany, których rolę klanową (`clans[key].roleId`) sam posiada; lider bez roli klanowej dostaje komunikat błędu. Uprawnienia i dozwolone klany są sprawdzane **ponownie przy wyborze z listy**. Komenda NIE ma `setDefaultMemberPermissions` (Discord nie pozwala domyślnie ograniczyć komendy do roli), więc widzą ją wszyscy, a nieuprawnieni dostają odmowę z handlera
