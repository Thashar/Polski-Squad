...
14. **Captcha relay (Lunar Details)** - `captchaSolverService.js`: formularz "Lunar Details" na garrytools.com (używany przez `getGroupId()` dla `/lunarmine`, `/analyse`, `/lme-snapshot`, przycisk szczegółów w `/rivals`) jest chroniony Google reCAPTCHA v2 (checkbox + wyzwanie obrazkowe 3×3 lub 4×4, escaluje natychmiast po kliknięciu checkboxa). `openLunarDetailsPage()` łączy się przez losowe proxy z `proxy.txt` (ten sam singleton `ProxyService` co reszta bota - IP serwera jest blokowane przez Cloudflare bez proxy, przez co formularz w ogóle się nie renderuje), przez `--proxy-server` w Puppeteer + `page.authenticate()` dla danych logowania; do 3 prób z różnymi proxy, zanim strona zostanie uznana za nieosiągalną. `dismissCookieConsent()` klika przycisk "Zgadzam się" natywnym DOM `.click()` przez `page.evaluate()` (nie Puppeteer `ElementHandle.click()`, który w headless_shell z `@sparticuz/chromium` czasem rzuca "Node is either not clickable or not an Element" mimo widocznego przycisku) - jeśli okno zgody na cookies zostanie otwarte, blokuje kliknięcia we wszystko pod spodem (w tym checkbox reCAPTCHA), więc jego niezawodne zamknięcie jest krytyczne. Sam checkbox reCAPTCHA nadal klikany jest prawdziwym kliknięciem Puppeteer (nie JS `.click()`), bo reCAPTCHA odróżnia zaufane kliknięcia myszy od syntetycznych wywołań JS w ramach wykrywania botów. Po wypełnieniu 4 Guild ID i kliknięciu checkboxa reCAPTCHA, wyzwanie obrazkowe przekazywane jest do rozwiązania: zrzut ekranu widgetu (`iframe[src*="bframe"]`) numerowany przez sharp+SVG na podstawie prawdziwych bounding boxów kafelków (`.rc-imageselect-tile`, odporne na wysokość nagłówka/stopki), przyciski 1-N + ✅ Zatwierdź/❌ Anuluj (max 5 rows/25 przycisków - limit Discorda). Widoczność zależy od `context` przekazanego do `getGroupId(guildIds, context)`: `{ interaction }` → wyzwanie wysyłane jako **ephemeral followUp** (widoczne tylko dla wywołującego, używane przez `/lunarmine`, `/analyse`, przycisk `/rivals`, ręczny `/lme-snapshot`); `{ channel, invokerId }` → zwykła wiadomość na kanale (używane tylko przez cotygodniowy cron, który nie ma interakcji Discorda) - cron wysyła prośbę na dedykowany kanał admina `adminCaptchaChannelId` (env `GARY_ADMIN_CAPTCHA_CHANNEL_ID`, domyślnie `1263240344871370804`), pobierany przez `client.channels.fetch()` w `index.js`, oddzielny od wątku wyników; jeśli fetch się nie powiedzie, spada z powrotem na wątek wyników. Do 8 rund, deadline 360s łącznie na wszystkie rundy (pierwotne 100s okazało się za mało dla realnego rozwiązywania przez człowieka w kilku rundach - webhook token interakcji Discord jest ważny 15 min, więc jest zapas), 90s na rundę. Po każdej rundzie (kliknięciu "Weryfikuj" oraz po pierwszym kliknięciu checkboxa) bot czeka na `page.waitForNetworkIdle()` + bufor 700ms przed zrobieniem zrzutu ekranu - reCAPTCHA w trybie dynamicznym podmienia część kafelków na nowe obrazki po zaznaczeniu ("Please also check the new images"), a zbyt wczesny zrzut łapał kafelki w trakcie ładowania (biały placeholder z checkmarkiem zamiast zdjęcia). Po rozwiązaniu klika "Show Details" i wyciąga prawdziwe Group ID z URL przekierowania (`extractGroupIdFromUrl`) - dalsza część (`fetchGroupDetails`) działa jak wcześniej przez axios+proxy, bo strony `/detail/{id}` nie mają captchy. Cron nie gwarantuje, że ktoś zdąży rozwiązać captchę w porę - w takim wypadku tylko snapshot TOP500 (osobny endpoint bez captchy) i tak się zapisze. Przyciski captchy (`captcha_tile_N`, `captcha_submit`, `captcha_cancel`) są obsługiwane wyłącznie przez dedykowany `message.createMessageComponentCollector()` w `askForSelection()` - główny router przycisków (`interactionHandlers.js` → `handleButtonInteraction()`) ma wczesny bypass na prefiks `captcha_` (return bez odpowiedzi), bo inaczej ten globalny handler (starszy listener na `interactionCreate`, rejestrowany raz w `index.js`) łapał kliknięcie jako pierwszy i odpowiadał "❌ Unknown button!" (brak `::` w customId), konsumując interakcję zanim dotarła do kolektora - błąd objawiał się brakiem jakiejkolwiek reakcji/logów w konsoli, bo `i.update()` w kolektorze cicho się wywalał (teraz logowane przez `logUpdateError`)
