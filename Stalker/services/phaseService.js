...
class PhaseService {
    ...
    async downloadImage(url, sessionId, index) {
        ...
        logger.info(`[PHASE1] 📥 Pobrano zdjęcie do pamięci: ${(buffer.length / 1024).toFixed(0)} KB`);
        return buffer;
    }
    ...
    async processImagesBatch(sessionId, downloadedFiles, guild, member, publicInteraction) {
        ...
        try {
            ...
            if (error.isAPIOverloaded) {
                logger.warn(`[PHASE${session.phase}] ⚠️ API Gemini przeciążone po 10 próbach - kończę procedurę OCR`);
                ...
                if (ci) {
                    try {
                        ...
                        logger.warn(`[PHASE${session.phase}] ⚠️ Nie udało się zaktualizować embeda o przeciążeniu: ${e.message}`);
                    }
                }
                try {
    ...
    /**
     * Dopasowuje nicki odczytane przez AI do nicków roli klanowej (przydział 1:1).
     * Deleguje do wspólnego utila `nickMatcher` (współdzielonego z remind/punish).
    ...
    normalizePlayersToClanNicks(players, clanNicks) {
        return assignNicksToClan(players, clanNicks, logger);
    }
