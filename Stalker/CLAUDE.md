...
- **Okno czasowe:** tylko od **wtorku 18:00** do **środy 17:45** czasu polskiego (`config.cxBoss`, `reminderUsageService.getCxWindow()`)
- **⚠️ TYMCZASOWE - tryb bez limitów:** `config.cxBoss.unlimited = true` wyłącza okno czasowe ORAZ jednorazowość (nie rejestruje użyć) — do testów. Aby przywrócić limity, ustaw `unlimited: false` w `Stalker/config/config.js`
