...
- **Analiza AI w trybie CX:** dedykowany prompt `analyzeNicksImagesBatch(filepaths, clanNicks)` (wersja `extract-nicks-batch` v1) — AI zwraca **SAME NICKI** (jeden na linię, bez wyników), dopasowane do listy nicków Discord z roli klanowej. Parser `parseAINickResponse()`: bez walidacji wyników, toleruje numerację/markdown/doklejone wyniki, deduplikuje. (Opcja `anyScore` w `parseAIResponse()` pozostaje dostępna, ale CX już z niej nie korzysta)
