...
- **AI:** prompt `news-relay-summary` v2 — zwraca JSON `{"title","summary"}` po polsku (parser toleruje bloki kodu i tekst dookoła, fallback: cała odpowiedź jako summary). Retry na 503/429/500/sieciowe (5× exponential backoff, max 5s). **Reguła o czasie/datach:** daty, godziny i czas trwania są KOPIOWANE dokładnie w formie z oryginału (bez przepisywania na schemat "od <data> do <data>"), a AI ma ZAKAZ wstawiania placeholderów typu "[data i godzina]" — brakującą datę/godzinę po prostu pomija. **Reguła o nazwach własnych:** nazwy z gry (wydarzenia, tryby, przedmioty, waluty, postacie, gildie) w oryginale są zwykle pisane WIELKIMI LITERAMI albo Z Wielkich Liter — zostają w oryginalnym brzmieniu (BEZ tłumaczenia) i są ZAWSZE pogrubiane, np. **Lunar Mine Expedition**, **Transmute Core**. Polska odmiana idzie na słowa dookoła nazwy, nie na samą nazwę.
