...
- **Format wiadomości:** `# {emoji}**{nazwa}**{emoji} 🆔 {gameId}` → opcjonalny tekst wstępny → `## Informacje i wymagania:` → wiersze `▶ __…__: **…**` → `▶ __Punkty 1 Fazy LME__: **…**` → dodatkowe wiersze i blok Lider/Vice (`╰┈➤`) → kreska zamykająca.
  ...
  - ⚠️ **Dwie RÓŻNE głębokości wcięcia**: `indentRequirements` (domyślnie 1) dla dodatkowych wymagań i `indentMembers` (domyślnie 3) dla bloku Lider/Vice. Obie grupy stoją na innych poziomach zagnieżdżenia — jedna wspólna wartość wyrównałaby je do siebie i lista przestałaby się czytać jak drzewko.
