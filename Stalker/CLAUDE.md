...
1. **Kary OCR** - Dwa tryby:
   ...
   - **AI OCR (opcjonalny):** `aiOcrService.js` - Google Gemini API (Gemini Vision), analiza wyników graczy przez AI
     ...
     - **Tryb batch (`/faza1`, `/faza2`, `/remind`, `/punish`):** `analyzeResultsImagesBatch(imagePaths, clanNicks)` - wysyła WSZYSTKIE zdjęcia naraz w jednym zapytaniu (oszczędność czasu/tokenów). Do promptu dołącza listę nicków z roli klanowej Discord i prosi AI o dopasowanie odczytanych nicków ze screenów do najbliższego nicku Discord. Zwraca format `<nick Discord> - <wynik>`. Wersja promptu: `extract-results-batch` v4 — REGUŁY KRYTYCZNE w promptcie: AI wypisuje WYŁĄCZNIE graczy faktycznie widocznych na zdjęciach (zdjęcie zawiera tylko kilka nicków — lista w odpowiedzi ma być tak samo krótka; osoby z listy Discord bez dopasowania na zdjęciu są POMIJANE całkowicie, NIE dostają wyniku 0), przepisuje nick z listy znak po znaku (zakaz dodawania prefiksów typu "PL | ") i pomija graczy z nieczytelnym wynikiem zamiast wymyślać wartości. **v3 (cofnięte)** kazało AI zwracać KOMPLETNĄ listę klanu z zerami dla nieobecnych — to błędnie przypisywało 0 osobom, których nie było na screenie (np. gdy wiadomość zawiera jeden screen z kilkoma nickami). Reguła o dokładnym przepisywaniu nicków zostaje — bez niej Gemini dodawał zhalucynowany prefiks rozstrajający dopasowanie 1:1 (zamiana wyników między graczami, np. JU73K ↔ 7-Words).
...
- **Analiza AI w trybie CX:** dedykowany prompt `analyzeNicksImagesBatch(filepaths, clanNicks)` (wersja `extract-nicks-batch` v2 — te same REGUŁY KRYTYCZNE co batch wyników: tylko gracze widoczni na zdjęciach, nick z listy znak po znaku, bez prefiksów) — AI zwraca **SAME NICKI** (jeden na linię, bez wyników), dopasowane do listy nicków Discord z roli klanowej. Parser `parseAINickResponse()`: bez walidacji wyników, toleruje numerację/markdown/doklejone wyniki, deduplikuje. (Opcja `anyScore` w `parseAIResponse()` pozostaje dostępna, ale CX już z niej nie korzysta)
