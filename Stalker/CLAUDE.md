...
6. **Fazy Lunar** - `phaseService.js`: `/faza1` (lista), `/faza2` (3 rundy damage), `/wyniki` (TOP30 z paginacją tygodni), `/progres`, `/clan-status`, `/clan-progres` (progres TOP30 klanu z wykresem), `/img` (dodaj zdjęcie tabeli do Fazy 2). Po każdym zatwierdzeniu `/faza1` (i przy starcie bota) wywołuje `clanThresholdsExportService.exportClanThresholds()` → zapisuje `shared_data/clan_thresholds.json` z minimalnym maxScore per klan, używanym przez Rekrutera do dynamicznej kwalifikacji.
   ...
   - **Normalizacja nicków AI (`normalizePlayersToClanNicks()`):** Po odczycie AI każdy nick jest deterministycznie normalizowany do kanonicznego nicku Discord ze snapshotu klanu. Dopasowanie po Levenshteinie na **grafemach** (emoji = 1 znak, `_splitGraphemes()` + `Intl.Segmenter`); akceptacja gdy różnica ≤ **20%** znaków. Gwarantuje spójne nicki między rundami Fazy 2 (`sumPhase2Results` sumuje po stringu nicku) — bez tego ten sam gracz odczytany raz z ozdobnikiem, raz bez (`Cygolos` vs `Cygolos😎`) liczyłby się jako dwie osoby z rozdzielonymi punktami. Dodatkowo dedup w obrębie partii (kilka nicków AI → ten sam Discord nick = wpis z wyższym wynikiem, zapobiega fałszywym konfliktom). Stosowane w `/faza1`, `/faza2` i `/test`. Uwaga: ciężkie błędy OCR powyżej progu (np. `krępo` vs `Krzempo` = 57%) celowo NIE są scalane, by nie połączyć różnych graczy.
