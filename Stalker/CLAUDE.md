...
1. **Kary OCR** - Dwa tryby:
   ...
   - **AI OCR (opcjonalny):** `aiOcrService.js` - Google Gemini API (Gemini Vision), analiza wyników graczy przez AI
     ...
     - **Tryb batch (`/faza1`, `/faza2`, `/remind`, `/punish`):** `analyzeResultsImagesBatch(imagePaths, clanNicks)` - wysyła WSZYSTKIE zdjęcia naraz w jednym zapytaniu (oszczędność czasu/tokenów). Do promptu dołącza listę nicków z roli klanowej Discord i prosi AI o dopasowanie odczytanych nicków ze screenów do najbliższego nicku Discord. Zwraca format `<nick Discord> - <wynik>`. Wersja promptu: `extract-results-batch` v2 — REGUŁY KRYTYCZNE w promptcie: AI wypisuje WYŁĄCZNIE graczy widocznych na zdjęciach (zakaz zrzucania całej listy klanu z zerami), przepisuje nick z listy znak po znaku (zakaz dodawania prefiksów typu "PL | ") i pomija graczy z nieczytelnym wynikiem zamiast wymyślać wartości. Bez tych reguł Gemini potrafił zwrócić cały roster klanu z 0 dla nieobecnych i zhalucynowanym prefiksem, co rozstrajało dopasowanie 1:1 (zamiana wyników między graczami, np. JU73K ↔ 7-Words).
