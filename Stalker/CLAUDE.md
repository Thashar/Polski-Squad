...
1. **Kary OCR** - Dwa tryby:
   ...
   - **AI OCR (opcjonalny):** `aiOcrService.js` - Google Gemini API (Gemini Vision), analiza wyników graczy przez AI
     ...
     - **Tryb batch (`/faza1`, `/faza2`, `/remind`, `/punish`):** `analyzeResultsImagesBatch(imagePaths, clanNicks)` - wysyła WSZYSTKIE zdjęcia naraz w jednym zapytaniu (oszczędność czasu/tokenów). Do promptu dołącza listę nicków z roli klanowej Discord i prosi AI o dopasowanie odczytanych nicków ze screenów do najbliższego nicku Discord. Zwraca format `<nick Discord> - <wynik>`. Wersja promptu: `extract-results-batch` v3 — REGUŁY KRYTYCZNE w promptcie: AI wypisuje KAŻDY nick z listy Discord dokładnie raz (nieobecni na zdjęciach i nieczytelni dostają wynik 0 — lista w odpowiedzi jest zawsze KOMPLETNA), przepisuje nick z listy znak po znaku (zakaz dodawania prefiksów typu "PL | ") i nie wymyśla wyników. Bez reguły o dokładnym przepisywaniu nicków Gemini potrafił dodać zhalucynowany prefiks, co rozstrajało dopasowanie 1:1 (zamiana wyników między graczami, np. JU73K ↔ 7-Words).
