...
const config = {
    ...
    messages: {
        ...
        attackLevelQuestion: "Wybierz poziom ataku, który reprezentujesz!",
        osQuestion: "Jakiego używasz systemu operacyjnego do gry?",
...
const ROLE_GROUPS = {
    survivors: ['1326510262668234816', '1326511028967571517', '1326511801877135432'],
    platforms: ['1339552918730178591', '1339552767806537770'],
    attackLevels: [
        '1173987591834583052', '1190252992482508800', '1212372650463203348', 
        '1254550028169773086', '1268879784000225290', '1304872630494433330', 
        '1304872767358894150', '1326505392548741160', '1326505466829869108', 
        '1326505521964126312', '1386995713409552507', '1386995792795140139', 
        '1386995859375259678', '1386995918741573712', '1386995960651059332'
    ]
};
...
async function safeAddRole(member, roleId) {
    try {
        ...
        const roleGroup = Object.keys(ROLE_GROUPS).find(group => 
            ROLE_GROUPS[group].includes(roleId)
        );
        
        if (roleGroup) {
            const rolesToRemove = ROLE_GROUPS[roleGroup].filter(id => id !== roleId);
            console.log(`[ROLE] Usuwanie poprzednich ról z grupy ${roleGroup}:`, rolesToRemove);
            
            for (const removeRoleId of rolesToRemove) {
                const roleToRemove = member.guild.roles.cache.get(removeRoleId);
                if (roleToRemove && member.roles.cache.has(removeRoleId)) {
                    await member.roles.remove(roleToRemove);
                    console.log(`[ROLE] Usunięto rolę ${removeRoleId}`);
                }
            }
        }
...
client.on('interactionCreate', async interaction => {
    ...
    try {
        ...
        switch (interaction.customId) {
            ...
            case 'metallia':
            case 'joey':
                console.log(`[INTERACTION] Użytkownik ${interaction.user.username} wybrał survivora: ${interaction.customId}`);
                await handleSurvivorChoice(interaction);
                break;
            default:
                if (interaction.customId.startsWith('attack_')) {
                    console.log(`[INTERACTION] Użytkownik ${interaction.user.username} wybrał poziom ataku: ${interaction.customId}`);
                    await handleAttackLevel(interaction);
                } else if (interaction.customId === 'android' || interaction.customId === 'ios') {
                    console.log(`[INTERACTION] Użytkownik ${interaction.user.username} wybrał OS: ${interaction.customId}`);
                    await handleOSChoice(interaction);
                }
                break;
        }
...
async function extractOptimizedStatsFromImage(imagePath, userId) {
    try {
        ...
        // Sprawdzenie czy nick został znaleziony w pierwszych 3 linijkach
        if (!stats.playerNick) {
...
// Poprawiona funkcja ekstraktacji statystyk z warunkiem dla nicku w pierwszych 3 linijkach
function extractStatsFromLines(text) {
    ...
    // Znajdź nick TYLKO w pierwszych 3 linijkach
    ...
    // Szukaj ataku od następnej linii po tej z nickiem
    if (nickLineIndex >= 0) {
        console.log(`[OCR] Szukanie ataku zaczynając od linii ${nickLineIndex + 2}`);
        
        // Szukaj w kolejnych liniach po linii z nickiem
    ...
    // Jeśli nadal nie znaleziono ataku, ostatnia szansa - przeszukaj wszystkie liczby
    if (!characterAttack) {
...
async function handleSurvivorChoice(interaction) {
    const member = interaction.member;
    let roleId;
    let survivorName;
    ...
    switch (interaction.customId) {
        case 'master_yang':
            roleId = config.roles.masterYang;
            survivorName = 'Master Yang';
            break;
        case 'metallia':
            roleId = config.roles.metallia;
            survivorName = 'Metallia';
            break;
        case 'joey':
            roleId = config.roles.joey;
            survivorName = 'Joey';
            break;
    }

    console.log(`[SURVIVOR] Użytkownik ${interaction.user.username} wybrał survivora: ${survivorName}`);

    const info = userInfo.get(interaction.user.id);
    if (info) {
        info.survivor = survivorName;
        userInfo.set(interaction.user.id, info);
    }

    await safeAddRole(member, roleId);

    userStates.set(interaction.user.id, { step: 'survivor_chosen' });
    await showAttackLevelChoice(interaction);
}

async function showAttackLevelChoice(interaction) {
    console.log(`[ATTACK_LEVEL] Pokazuję wybór poziomu ataku dla ${interaction.user.username}`);
    
    const attackLevels = [
        { id: 'attack_100k', label: '100k+' },
        { id: 'attack_200k', label: '200k+' },
        { id: 'attack_300k', label: '300k+' },
        { id: 'attack_400k', label: '400k+' },
        { id: 'attack_500k', label: '500k+' },
        { id: 'attack_600k', label: '600k+' },
        { id: 'attack_700k', label: '700k+' },
        { id: 'attack_800k', label: '800k+' },
        { id: 'attack_900k', label: '900k+' },
        { id: 'attack_1000k', label: '1kk+' },
        { id: 'attack_1200k', label: '1,2kk+' },
        { id: 'attack_1400k', label: '1,4kk+' },
        { id: 'attack_1600k', label: '1,6kk+' },
        { id: 'attack_1800k', label: '1,8kk+' },
        { id: 'attack_2000k', label: '2kk+' }
    ];

    const rows = [];
    for (let i = 0; i < attackLevels.length; i += 5) {
        const row = new ActionRowBuilder();
        const levelSlice = attackLevels.slice(i, i + 5);
        
        levelSlice.forEach(level => {
            row.addComponents(
                new ButtonBuilder()
                    .setCustomId(level.id)
                    .setLabel(level.label)
                    .setStyle(ButtonStyle.Secondary)
                    .setEmoji('<:L_ATK:1209754263228522516>')
            );
        });
        rows.push(row);
    }

    await updateUserEphemeralReply(interaction.user.id, config.messages.attackLevelQuestion, rows);
}

async function handleAttackLevel(interaction) {
    const attackLevel = interaction.customId.replace('attack_', '');
    const roleId = config.roles.attack[attackLevel];
    
    console.log(`[ATTACK_LEVEL] Użytkownik ${interaction.user.username} wybrał poziom ${attackLevel}`);
    
    const info = userInfo.get(interaction.user.id);
    if (info) {
        info.attackLevel = attackLevel;
        userInfo.set(interaction.user.id, info);
    }
    
    if (roleId) {
        await safeAddRole(interaction.member, roleId);
    }

    await showOSChoice(interaction);
}

async function showOSChoice(interaction) {
    console.log(`[OS_CHOICE] Pokazuję wybór systemu operacyjnego dla ${interaction.user.username}`);
    ...
    const row = new ActionRowBuilder()
        .addComponents(
            new ButtonBuilder()
                ...
                .setLabel('Android')
                .setStyle(ButtonStyle.Success)
                .setEmoji('<:z_Android:1339558626851029033>'),
            new ButtonBuilder()
                .setCustomId('ios')
                .setLabel('iOS')
                .setStyle(ButtonStyle.Primary)
                .setEmoji('<:z_Apple:1339558754626043985>')
        );

    await updateUserEphemeralReply(interaction.user.id, config.messages.osQuestion, [row]);
}

async function handleOSChoice(interaction) {
    const member = interaction.member;
    const osChoice = interaction.customId === 'android' ? 'Android' : 'iOS';
    
    console.log(`[OS_CHOICE] Użytkownik ${interaction.user.username} wybrał system: ${osChoice}`);
    ...
    const osRoleId = interaction.customId === 'android' ? config.roles.android : config.roles.ios;
    
    await safeAddRole(member, osRoleId);
    await safeAddRole(member, config.roles.verified);

    await updateUserEphemeralReply(interaction.user.id, '✅ Proces rekrutacji zakończony pomyślnie! Witamy na serwerze!');

    console.log(`[OS_CHOICE] ✅ Zakończono rekrutację dla ${interaction.user.username}`);

    setTimeout(() => {
        userEphemeralReplies.delete(interaction.user.id);
        console.log(`[OS_CHOICE] Usunięto ephemeral reply dla ${interaction.user.username}`);
    }, 5000);

    userStates.delete(interaction.user.id);
    userInfo.delete(interaction.user.id);
}
...
async function handleImageInput(message, userState) {
    ...
    try {
        ...
        if (!stats || !stats.isValidEquipment) {
            ...
            // Sprawdzenie czy to błąd braku nicku w pierwszych 3 linijkach
...
async function assignClanRole(member, attack, user) {
    ...
    if (attack < 100000) {
        ...
        // Wysyłamy wiadomość notQualified na kanał welcome zamiast ephemeral reply
...
async function sendUserSummary(user, channelId) {
    ...
    if (info.survivor) {
        summaryText += `**Główny Survivor:** ${info.survivor}\n`;
    }

    if (info.attackLevel) {
        summaryText += `**Poziom ataku:** ${info.attackLevel}\n`;
    }

    if (info.os) {
        summaryText += `**System operacyjny:** ${info.os}\n`;
    }
