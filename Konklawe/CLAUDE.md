...
11. **Cenzura Zakazanej Frazy** - `messageHandlers.js`: Automatyczna cicha klątwa admina na **1 godzinę** za napisanie zakazanej frazy (domyślnie "fortnite", konfigurowana przez `KONKLAWE_FORBIDDEN_PHRASE` w .env) lub jej prób obejścia. Metoda `detectForbiddenPhrase(text, phrase)` generyczna - działa dla dowolnej frazy. Algorytm: leet speak (0→o, 1→i, 3→e, 7→t...), homoglify unicode, separatory (f.o.r.t.n.i.t.e), podwojone litery (forrtniite→fortnite), spacja wewnątrz frazy, fonetyczne zamienniki (ph→f, y→i, kn→n). Wywołuje `applyRandomCurseToUser` z czasem 60min. Brak publicznego powiadomienia. Wykluczone kanały: `channels.fortniteCensorExcluded`.
