...
- **Podgląd w Centrum Dowodzenia:** embed `⚙️ Narzędzia` ma pole `📤 Rankingi na stronie` — czas ostatniej wysyłki, jej rodzaj (pełny snapshot / pojedynczy serwer / usunięcie serwera) i liczba śledzonych serwerów. Dane z `webRankingSyncService.getStatus()`; stan `lastSync` leży w tym samym pliku co skróty, więc przeżywa restart
...
- **Rząd 1 — liczniki, ZAWSZE szare (`Secondary`): 4 najczęstsze reakcje z ikoną + zbiorczy `➕` na końcu** (`TOP_BUTTONS = 4`). Do zbiorczego wpadają reakcje poza czołową czwórką. Suma przycisków zawsze równa się sumie reakcji, a całość mieści się w jednym rzędzie niezależnie od liczby różnych emotek
...
- **Rząd 2 — ostatnia reakcja:** `<nick> z <nazwa serwera>` / `<nick> from <server name>`, z **emotką tej reakcji jako ikoną**. Nick to **nick serwerowy** (`member.displayName`) z serwera, na którym kliknięto reakcję; nazwa serwera z cache'u Discorda (zapas: tag z konfiguracji)
  ...
  - Emotka z serwera bez bota → ikona zastępcza `💬` (inaczej Discord odrzuciłby cały komponent i edycja wiadomości by padła)
...
- **Klik w licznik → ephemeral z listą osób** (`collectReactors` + `_handleBroadcastReactionButton`): kto zareagował, pogrupowany po serwerze, z liczbą przy nazwie serwera. Boty pomijane
...
- **Przycisk „ostatnia reakcja" pozostaje bezczynny**, ale klik i tak MUSI zostać potwierdzony (`deferUpdate`) — **bez tego Discord pokazuje „This interaction failed"**; przycisk `disabled` odpada, bo nie byłby klikalny
