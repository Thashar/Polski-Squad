...
- **Rzędy 1-4 — liczniki, ZAWSZE szare (`Secondary`): 19 najczęstszych reakcji z ikoną + zbiorczy `➕` na 20. slocie** (`TOP_BUTTONS = 19`). Do zbiorczego wpadają reakcje poza czołową dziewiętnastką. **`➕` jest widoczny ZAWSZE**, także z licznikiem 0 — układ przycisków ma być stały, żeby liczniki nie przeskakiwały pod palcem między odświeżeniami. Suma przycisków zawsze równa się sumie reakcji; 20 przycisków = dokładnie 4 rzędy (Discord: 5 przycisków/rząd, 5 rzędów/wiadomość), piąty rząd zostaje dla „ostatniej reakcji"
