...
7. **System Osiągnięć** — `achievementService.js` + `config/achievements.js`:
   ...
   - **Jednorazowa naprawa z backupu:** `services/achievementRestoreService.js` — `buildRestorePlan(backupGuildsRoot)` (dry-run) + `applyRestorePlan(plan)`. Scala brakujące osiągnięcia `unlocked` (kategorie != `score`) z rozpakowanego backupu do aktualnych plików (odzysk po race condition) i przelicza osiągnięcia `score` z aktualnego `ranking.json` (naprawa rotacji `score_100xx` → `score_100sp`). Osiągnięcia są tylko dodawane (wyjątek: usunięcie błędnego `score_100xx` gdy wynik < 1e29). Przed zapisem kopia `achievements.before_restore_<ts>.json`, zapis atomowy. Uruchamiane przez Muteusza komendą `/napraw-osiagniecia-ee` (pobiera backup z Google Drive).
