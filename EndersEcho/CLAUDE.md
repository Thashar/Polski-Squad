...
6. **Panel Admina** — dostępny przez `/manage`:
   ...
   - **Automatyczna retencja danych (30 dni)** — `guildDataRetentionService.js`: `guildDelete` zapisuje serwer do `data/pending_guild_deletions.json` (z nazwą, językiem i timestampem); sweep przy starcie + co 12 h usuwa dane serwerów, dla których minęło 30 dni (ten sam zakres co panelowy przycisk: `data/guilds/{guildId}/` + wpis w `guild_configs.json`). **`data/token_usage.json` celowo nietykane** — statystyki tokenów AI zostają do celów rozliczeniowych/statystycznych (sekcja 7 polityki prywatności). `guildCreate` anuluje oczekujące usunięcie (bot wrócił); sweep też anuluje wpisy serwerów obecnych w cache (osierocone przy downtime). Po faktycznym usunięciu — powiadomienie na kanał logów serwerowych (`sendAdminNotification`, ping do head admina) z listą co usunięto/zachowano, w języku zapamiętanym przy `guildDelete`. Błąd usuwania nie kasuje wpisu — retry przy kolejnym przebiegu. Zgodne z deklaracją 30 dni w polityce prywatności (endersecho.thashar.dev/privacy).
