...
- **Podgląd w Centrum Dowodzenia:** embed `⚙️ Narzędzia` ma pole `📤 Rankingi na stronie` — czas ostatniej wysyłki, jej rodzaj (pełny snapshot / pojedynczy serwer) i liczba śledzonych serwerów. Dane z `webRankingSyncService.getStatus()`; stan `lastSync` leży w tym samym pliku co skróty, więc przeżywa restart
