...
3. **Role TOP (opcjonalne)** - `roleService.js`:
   ...
   - **Cykliczny raport Global TOP10** (`globalTop10Service`) — `services/globalTop10Service.js`:
     ...
     - Konfiguracja przez panel admina → **📅 Interwał TOP10** (tylko head admin) → modal z 2 polami: data/godzina punktu odniesienia (format `DD.MM.RRRR GG:MM`) i **numer bossa w sezonie (1-9, domyślnie 1)**; puste pole daty = wyłącz harmonogram
     - **Numer bossa w sezonie (`reportNumber` w `setSchedule(iso, reportNumber)`):** pozwala zrekalibrować cykl, gdy punkt odniesienia to NIE pierwszy boss sezonu — np. jeśli dziś kończy się 7. boss trwającego sezonu, admin wpisuje dzisiejszą datę + `7`, a `triggerCount` zostaje ustawiony na `reportNumber - 1` zamiast zawsze na 0. Bez tego harmonogram zawsze zakładał "pierwszy raport" i błędnie przesuwał kolejne 4-dniowe przerwy względem realnego sezonu.
     - **`setSchedule()` nie resetuje pozycji w cyklu, gdy data i numer bossa się nie zmieniają** — samo otwarcie i zatwierdzenie modala z tymi samymi (prefilled) wartościami nie zeruje już `triggerCount`. Wcześniej każde zatwierdzenie modala (nawet bez zmiany danych, np. tylko żeby podejrzeć harmonogram) bezwarunkowo zerowało `triggerCount`, co po cichu przesuwało pozycję 4-dniowej przerwy względem realnego końca sezonu.
     - **Podana data może być w przeszłości** — traktowana jest jako punkt odniesienia (np. faktyczny, znany koniec bossa), a harmonogram (`setSchedule()`) sam przewija się wg wzorca 9×3 dni + 4 dni przerwy do najbliższego przyszłego terminu (`_stepOnce()` w pętli), **bez wysyłania** pominiętych po drodze raportów — pozwala to poprawnie zrekalibrować cykl po wykryciu rozjazdu, wpisując realną, znaną datę (i numer bossa) zamiast liczyć ręcznie następny przyszły termin. Potwierdzenie w panelu pokazuje realnie wyliczony najbliższy termin po przewinięciu.
...
4. **Paginacja + Wybór Rankingu** - `interactionHandlers.js`:
   ...
   - **Wykres historii rekordów** (`scoreHistoryService` + `chartService`): jeśli wywołujący ma ≥ 2 wpisy w ciągu ostatnich 90 dni → PNG dołączony do tej samej wiadomości rankingowej. Oś X: daty rzeczywiste (max 3 miesiące), oś Y: wyniki z jednostkami (K/M/B/T/Q/Qi/Sx/Sp), kropki z wynikiem nad każdym. Dane persystowane w `data/guilds/{guildId}/wyniki/{userId}.json` — każde pobicie rekordu to nowy wpis. **Wykres budowany z historii ze WSZYSTKICH serwerów** (`getUserHistoryAllGuilds`) — wyniki z różnych serwerów scalane chronologicznie. Ranking serwera X usuwa gracza gdy ma lepszy wynik na innym serwerze (tylko ranking.json — historia wyników zostaje).
