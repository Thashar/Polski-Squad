...
1. **OCR Wyników** - Dwa tryby:
   ...
   - **AI OCR (opcjonalny):** `aiOcrService.js` - Google AI API (Gemini Vision), dwuetapowa walidacja
     ...
     - **Wycinek zrzutu odrzucany przez wzorzec** (`compare-template` **v6**): prompt ma osobną sekcję `KADR` — kadr obcięty tak, że nie widać całego panelu z banerem i nazwą przeciwnika (albo brak tła z gameplayem) → `NOK`, nawet gdy same linie Best/Total wyglądają poprawnie
