...
- **Komenda `/profiles`** (PL: `/profile`… zarejestrowana jako `profiles` z lokalizacją `profile`) — panel: lista profili z wynikami i pozycjami globalnymi, `➕ Dodaj profil` (modal nazwy), `📌 Ustaw profil do śledzenia`, `✏️ Zmień nazwę`, `🗑️ Usuń profil` (potwierdzenie → kasuje wpis rankingowy, rekordy bossów, historię, osiągnięcia i subskrypcje profilu na wszystkich serwerach + aktualizuje role TOP)
- **Profil do ŚLEDZENIA** (`active` w `profiles.json`, `getActiveIndex` / `getActivePlayerKey`) — wybór gracza, który profil bot pokazuje jako „jego" w widokach personalnych:
  - Ustawiany przyciskiem `📌 Śledź ten profil` w `/profile` (rząd przełączania profili) oraz z panelu `/profiles`. Przycisk **pojawia się dopiero gdy gracz ma więcej niż jeden profil** i na aktualnie śledzonym profilu jest wyłączony (zielony `📌 Śledzony`)
...
- **Własny profil — Rząd profili (tylko gdy gracz ma >1 profil):** `🏠 Main` | `② …` | `③ …` (max 4 przyciski `profile_view_{index}`, aktualnie oglądany Primary + disabled) oraz `📌 Śledź ten profil` / `📌 Śledzony` (`profile_track`, Success + disabled gdy ten profil już jest śledzony). Kliknięcie `profile_track` zapisuje wybór w `profiles.json` (`setActive`), wysyła ephemeral potwierdzenie i przerysowuje przyciski. Przycisk działa tylko na własnym profilu — na cudzym rząd profili się nie pojawia
...
- **CustomIDs:** `profile_main` | `profile_bosses` | `profile_bosses_prev` | `profile_bosses_next` | `profile_ach_overview` | `profile_ach_cat_{key}` | `profile_search` | `profile_search_modal` | `profile_search_sel` | `profile_back` | `profile_manage_subs` | `profile_subscribe` | `profile_unsubscribe` | `profile_view_{index}` | `profile_track` (wszystkie routowane przez whitelistę w `handleButtonInteraction` — nowy customId `profile_*` MUSI tam trafić, inaczej przycisk nie zadziała)
