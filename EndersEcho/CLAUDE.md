...
1. **OCR Wyników** - Dwa tryby:
   ...
   - **AI OCR (opcjonalny):** `aiOcrService.js` - Google AI API (Gemini Vision), dwuetapowa walidacja
     ...
     - **Walidacja NAZWY BOSSA** (`isUnreadableBossName` w `config/bossNames.js`, wołane w `parseAIResponse`): pierwsza linia odpowiedzi musi być NAZWĄ, a nie zdaniem. Odrzucane (`error: 'BOSS_NAME_UNREADABLE'`, `isValidVictory: false`) są: frazy odmowy modelu (`nie udało`, `nie zident`, `cannot`, `unable to`…), znaki zastępcze (`BRAK`, `unknown`, `N/A`, `0`, `-`), tekst dłuższy niż 48 znaków albo 6 słów oraz wieloczłonowy tekst zakończony kropką. Prompt (`extract-data-eng` **v5**) każe modelowi w takiej sytuacji wpisać samo `BRAK` zamiast tłumaczyć się zdaniem
     - **Warunek banera „Victory" (prompt v5):** prompt mówi, że poprawny screen ma u góry panelu baner z napisem „Victory", a bez tego napisu model ma odpowiedzieć jedną linią `BRAK VICTORY` – parser łapie ją istniejącym słowem kluczowym `brak victory` → `INVALID_SCREENSHOT`. Powód: ekran „Total Damage" (rozbicie obrażeń na bronie/pupile z procentami) był przyjmowany jako poprawny i model wymyślał z niego bossa, Best i Total. ⚠️ Celowo TYLKO warunek pozytywny (co ma być), bez opisywania, jak wygląda zły ekran – Flash Lite z negatywnych przykładów halucynuje
...
3. **Role TOP (opcjonalne)** - `roleService.js`:
   ...
   - **Cykliczny raport Global TOP10** (`globalTop10Service`) — `services/globalTop10Service.js`:
     - **Raport na KONIEC każdego bossa** (model `SCHEDULE_VERSION = 2`): sezon = 9 bossów po 3 dni + 1 dzień przerwy = 28 dni (`SEASON_MS`). `firstTrigger` = **start sezonu (początek 1. bossa)**, a raport k-tego bossa (k = 1…9) idzie o `start + 28 dni × sezon + 3 dni × k` (`_reportTimeAt(index)`). Raporty wypadają więc w dniach 3, 6, …, 27, a 4 dni mijają między raportem za 9. bossa (dzień 27) i za 1. bossa nowego sezonu (dzień 31). Termin jest czystą funkcją `firstTrigger` i numeru raportu — `triggerCount` to numer NASTĘPNEGO raportu, `nextTrigger` jego termin (`_setIndex`)
       - ⚠️ **Model 1 (do września 2026) wysyłał raport #1 w chwili STARTU sezonu**, a przerwę dawał po 9. raporcie. Wszystkie raporty poza jednym trafiały dobrze, ale raport za ostatniego bossa szedł na starcie nowego sezonu — **dzień po końcu bossa**, bo „4-dniowe” okno to w rzeczywistości 3 dni bossa + 1 dzień przerwy. `_migrateSchedule()` przy starcie przelicza zapisany stan raz (znacznik `scheduleVersion`): następny raport = pierwszy termin nowego modelu po ostatnim WYSŁANYM raporcie starego, więc żaden termin nie ginie, a raport za ostatniego bossa przychodzi o dzień wcześniej. Gdy ten termin już minął (wdrożenie w dzień przerwy), `_tick` wysyła go od razu
     - **`CYCLE_LEN = 9`** (liczba raportów = bossów w sezonie, NIE liczba wszystkich pozycji cyklu) — poprzednio błędnie ustawione na 10, co wstawiało dodatkowy, 10. raport przed każdą kolejną przerwą; efekt: **każdy sezon po pierwszym miał w rzeczywistości 10 raportów zamiast 9**, a harmonogram trwale przesuwał się o 3 dni w przód przy każdej kolejnej granicy sezonu. Zweryfikowane symulacją względem realnego kalendarza (27 raportów / 3 sezony) — po poprawce wszystkie daty zgadzają się co do dnia.
     - Stopka embeda „Next report in X days” = `_reportTimeAt(triggerCount + 1) − _reportTimeAt(triggerCount)` — odstęp między wysyłanym raportem a następnym, z tego samego wzoru co harmonogram
     - Konfiguracja w `data/global_top10_config.json` (enabled, firstTrigger, nextTrigger, triggerCount, scheduleVersion, lastSnapshot)
     ...
     - **Brak serii raportów po przerwie bota:** `_advanceTrigger()` po wysłaniu przewija BEZ wysyłania wszystkie terminy, które już minęły (log `Pominięto N zaległych terminów`). Wcześniej przesuwał o jeden krok, więc po wyłączeniu bota na ponad 3 dni każdy zaległy termin wysyłał osobny raport — co minutę, z samymi `=` przy graczach
     - Konfiguracja przez panel admina → **📅 Interwał TOP10** (tylko head admin) → modal z jednym polem: **start sezonu — początek 1. bossa** (format `DD.MM.RRRR GG:MM`); pierwszy raport idzie 3 dni później, na koniec tego bossa; puste pole = wyłącz harmonogram. **Podpowiedź w polu = start BIEŻĄCEGO sezonu** (`getCurrentSeasonStart()`), nie najbliższy raport. Potwierdzenie pokazuje start sezonu i najbliższy raport
     ...
     - **`setSchedule()` nie przestawia harmonogramu, gdy układ sezonów się nie zmienia** — data różniąca się od zapisanego `firstTrigger` o pełną wielokrotność 28 dni (z tolerancją ±2 h na zmianę czasu letni/zimowy, bo podpowiedź pokazywana jest w czasie polskim) zwraca `false` i niczego nie rusza; potwierdzenie mówi wtedy „bez zmian”. ⚠️ **Wcześniej modal podpowiadał `nextTrigger` (najbliższy RAPORT), a strażnik działał tylko przy `triggerCount === 0`** — otwarcie i zatwierdzenie modala bez zmian w trakcie sezonu traktowało najbliższy raport jak raport #1 i przesuwało przerwę nawet o kilkanaście dni.
     - **Podana data może być w przeszłości** — traktowana jest jako punkt odniesienia (np. faktyczny, znany początek cyklu), a harmonogram (`setSchedule()`) ustawia najbliższy przyszły termin (`_firstIndexAfter(now)`; dla startu sezonu w przyszłości — pierwszy raport tego sezonu), **bez wysyłania** pominiętych po drodze raportów — pozwala to poprawnie zrekalibrować cykl po wykryciu rozjazdu, wpisując realną, znaną datę zamiast liczyć ręcznie następny przyszły termin. Potwierdzenie w panelu pokazuje realnie wyliczony najbliższy termin po przewinięciu.
     ...
     - **Historia raportów — `data/global_top10_history.json`** (`_appendHistory`, `_loadHistory`): jeden wpis na ogłoszenie z pozycjami, **nickami i serwerami**. ⚠️ Nicki zapisujemy razem z pozycjami, bo gracz może zniknąć z rankingu albo skasować profil, a wykres sprzed dwóch miesięcy ma nadal wiedzieć, kogo rysuje — odtworzenie nazwy z bieżącego rankingu dałoby dla takiej osoby puste miejsce w legendzie. Konfiguracja (`lastSnapshot`) trzyma wyłącznie OSTATNI raport, więc bez tego pliku nie da się narysować niczego wstecz. Historia przycinana do okna 84 dni i `MAX_HISTORY_REPORTS`
       - **Zasianie wstecz przy starcie** (`_seedHistoryOnce`, jednorazowo gdy plik pusty): terminy z harmonogramu (`_pastReportTimes` — `_reportTimeAt` dla numerów poprzedzających `triggerCount`), pozycje z `reconstructTop10At` w `backfill-position-history.js`. Bez tego wykres byłby pusty przez pierwsze ~3 miesiące po wdrożeniu. ⚠️ **Punkty odtworzone są PRZYBLIŻENIEM** (`reconstructed: true`): nie biorą udziału gracze, którzy od tamtej pory wypadli z rankingu, a `_pastReportTimes` odtworzy błędne terminy, jeśli harmonogram był po drodze przestawiany. Prawdziwe ogłoszenia dopisywane od teraz są dokładne i z czasem wypchną odtworzone poza okno
...
| `cc_server_config` | Podgląd konfiguracji serwera — select skonfigurowanych serwerów (`cc_srvcfg_sel`, paginacja `cc_srvcfg_pg_{n}`) → ephemeral embed z pełnym wpisem z `guild_configs.json` + rankingi ról (head admin) |
...
| `panel_diagnostics` | Raport uprawnień bota: serwer, kanał bota, realny test dostarczenia `/info` (embed próbny wysyłany i kasowany), kanały raportów, hierarchia ról TOP, **komendy slash**, intenty — dostępny w `/configure` gdy wszystkie kroki ukończone (każdy admin) |
...
- **Krok 6:** Powiadomienia Global TOP10 (Tak/Nie) — per-guild flaga `globalTopNotifications` (backward compat: odczytuje też stare `globalTop3Notifications`). Stan kreatora trzyma ją pod starą nazwą `state.globalTop3Notifications`, ale ZAPIS idzie jako `globalTopNotifications` — każde miejsce czytające zapisaną konfigurację musi brać `globalTopNotifications ?? globalTop3Notifications` (tak robią `guildConfigService.getAllConfiguredGuilds()`, `config.getGuildConfig()` i powiadomienie o konfiguracji na kanale logów, helper `raportyTop10Wlaczone`). ⚠️ Wcześniej log konfiguracji czytał samą starą nazwę: pierwsza konfiguracja zawsze pokazywała „✅ Włączone”, a zmiana ustawienia przy rekonfiguracji nie trafiała do listy zmian (gdy była jedyną zmianą, powiadomienie w ogóle nie szło)
...
- Zielony przycisk **✅ Zaakceptuj konfigurację!** pojawia się gdy wszystkie kroki ukończone; obok niego pojawia się wtedy też przycisk **🔍 Diagnostyka** (`panel_diagnostics`) — dostępny dla każdego administratora, sprawdza uprawnienia bota (serwer + kanał bota + rozgłoszenia `/info` + kanały raportów + hierarchia ról TOP + intenty). **Stan czytany jest z API, nie z cache**: `_buildDiagnosticsEmbed` jest `async` i pobiera membera bota (`guild.members.fetchMe({force:true})`), role (`guild.roles.fetch()`) oraz każdy kanał (`client.channels.fetch(id,{force:true})`). Powód: nakładki uprawnień odświeżają się zdarzeniem `channelUpdate`, które przychodzi tylko dla kanałów widocznych dla bota — po odebraniu **ViewChannel** zdarzenia milkną, w cache zostaje stara zielona wersja i diagnostyka pokazywała komplet ✅, podczas gdy realna wysyłka leciała na `Missing Access`.
  ...
  - **⌨️ Sekcja `Komendy slash`** — `rest.get(Routes.applicationGuildCommands(clientId, guildId))`, czyli pytanie do API, co REALNIE jest zarejestrowane na tym serwerze. ⚠️ **Komendy slash nie zależą od żadnego uprawnienia roli, tylko od scope OAuth `applications.commands` nadanego przy zapraszaniu bota.** Bot zaproszony samym scope'em `bot` siedzi na liście członków, pisze, reaguje i przechodzi CAŁĄ resztę diagnostyki na zielono, a mimo to nie ma tam ani jednej komendy — objaw wygląda jak „inne boty mają komendy, nasz nie". Rejestracja zwraca wtedy 403/50001, co do tej pory lądowało wyłącznie w logu (`registerSlashCommands` łapie błąd per serwer), więc admin serwera nie miał tego jak zobaczyć. Trzy stany raportu: komendy są (lista nazw + podpowiedź o Ustawienia serwera → Integracje, gdzie admin może odebrać dostęp rolom/kanałom), pusta lista (rejestracja nie przeszła — restart bota ją powtarza) oraz 403/50001 (brak scope — bota trzeba zaprosić PONOWNIE linkiem ze `scope=bot%20applications.commands`, wchodzi na dotychczasową rolę i nic nie traci)
...
- **przy KAŻDYM `rankingService.saveRanking()`** — to jedyne miejsce, przez które przechodzi każda
  ...
  (`_syncInFlight` = promise trwającego przebiegu). Wywołanie bez rankingu w trakcie przebiegu
  ustawia `_syncAgain` — po zakończeniu leci jeszcze jeden przebieg na świeżym rankingu
...
- **przed zbudowaniem raportu TOP 10** (`_sendReports`, `buildOnDemandEmbed`) — z gotowym rankingiem
  ...
  ⚠️ Wywołanie Z rankingiem **doczekuje** trwający przebieg i dopiero potem się wykonuje — wcześniej
  było pomijane, gdy raport trafił w 10-minutowy przebieg timera, i część graczy dostawała „Nowa pozycja"
...
- `🧾 cc_server_config` → select skonfigurowanych serwerów (`cc_srvcfg_sel`, paginacja `cc_srvcfg_pg_{n}` przez `_buildServerPickerRows`) → embed `_buildServerConfigEmbed(guildId, cfg, client)` z pełnym zrzutem wpisu z `guild_configs.json`: kanał bota, kanał raportów, język/tag, blokady OCR per komenda, raporty Global TOP10, auto-reakcja, role TOP (progi + role, także stary format `top1…top11to30` i stan `disabled`), rankingi ról (`role_rankings.json`), weryfikacja społeczności (próg + kanał), moderatorzy gry, kto i kiedy konfigurował (`configuredBy`, czas warszawski), flagi (`importedFromEnv`, `newServerAnnounced`, ban, bot nieobecny). Pod embedem przycisk `◀️ Inny serwer` (`cc_srvcfg_pg_0`) wraca do listy. **Lista obejmuje też serwery, z których bot wyszedł** (`_ccConfiguredServersAll`, podpowiedź `⚪ bot nieobecny`) — konfiguracja żyje jeszcze 30 dni retencji, a właśnie wtedy nie da się jej sprawdzić z poziomu serwera. ⚠️ **Nazwy kanałów, ról i osób rozwiązywane ręcznie z cache** (`#nazwa (id)`), nie przez wzmianki `<#id>`/`<@&id>` — embed ląduje na kanale Centrum Dowodzenia, czyli na INNYM serwerze, gdzie Discord wyrenderowałby je jako „nieznany kanał"
