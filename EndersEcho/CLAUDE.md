...
- **Rząd 2 — ostatnia reakcja:** `<nick> z serwera <tag> zostawił reakcję` / `<nick> from <tag> left a reaction`, z **emotką tej reakcji jako ikoną**. Nick to **nick serwerowy** (`member.displayName`) z serwera, na którym kliknięto reakcję; tag pochodzi z konfiguracji tego serwera. Etykieta przycinana do 80 znaków (limit Discorda)
