...
- **Limit klikania liczników: 5 kliknięć w 5 s → 15 minut przerwy** (`BCR_WINDOW_MS`, `BCR_MAX_CLICKS`, `BCR_PENALTY_MS` w `interactionHandlers.js`, stan per użytkownik w `_bcrClicks`, sprzątany po przekroczeniu 500 wpisów). Blokada współbieżności chroni przed tłumem, a to przed jedną osobą walącą w przycisk. W trakcie przerwy KAŻDE kliknięcie dostaje ephemeral `broadcastVoteCooldown` (pol/eng) z relatywnym czasem końca — martwy przycisk bez wyjaśnienia wyglądałby jak awaria
