...
- Zielony przycisk **✅ Zaakceptuj konfigurację!** pojawia się gdy wszystkie kroki ukończone; obok niego pojawia się wtedy też przycisk **🔍 Diagnostyka** (`panel_diagnostics`) — dostępny dla każdego administratora, sprawdza uprawnienia bota (serwer + kanały + hierarchia ról TOP). **Stan czytany jest z API, nie z cache**: `_buildDiagnosticsEmbed` jest `async` i pobiera membera bota (`guild.members.fetchMe({force:true})`), role (`guild.roles.fetch()`) oraz każdy kanał (`client.channels.fetch(id,{force:true})`). Powód: nakładki uprawnień odświeżają się zdarzeniem `channelUpdate`, które przychodzi tylko dla kanałów widocznych dla bota — po odebraniu **ViewChannel** zdarzenia milkną, w cache zostaje stara zielona wersja i diagnostyka pokazywała komplet ✅, podczas gdy realna wysyłka leciała na `Missing Access`. Nieudany `fetch` jest teraz odpowiedzią: `50001` → „bot nie widzi kanału" + podpowiedź, `10003` → „kanał nie istnieje" + odesłanie do `/configure`. **Kontrola przynależności**: `channels.fetch` szuka globalnie po wszystkich serwerach bota, więc kanał o `guildId` innym niż sprawdzany serwer jest odrzucany z komunikatem „należy do INNEGO serwera" — liczenie na nim `permissionsIn` membera z tego serwera dawało wynik bez znaczenia, potrafiący wyjść zielony. W `_handleInfoSend` nieudana wysyłka loguje dodatkowo twarde fakty (`[/info] Diagnoza …`): `guildId` kanału kontra id serwera, typ kanału, czy da się na niego wysłać i pełną listę uprawnień bota — to rozstrzyga między odebranym ViewChannel, kanałem z obcego serwera i kanałem nietekstowym
