...
class InteractionHandler {
    ...
    async handleSelectMenuInteraction(interaction) {
        try {
            ...
            if (customId === 'ee_unblock_select') {
                ...
                if (success) {
                    await interaction.update({
                        content: `✅ Odblokowano użytkownika **${username}**.`,
                        embeds: [],
                        components: []
                    });
                } else {
                    await interaction.update({ content: '⚠️ Użytkownik nie był zablokowany.', embeds: [], components: [] });
                }
                return;
            }
    ...
    async _sendInvalidScreenReport(interaction, imagePath, reason, gl) {
        ...
        try {
            ...
            const row = new ActionRowBuilder().addComponents(approveBtn, blockBtn);

            await channel.send({ embeds: [embed], files: [fileAttachment], components: [row] });
