...
4. **Subskrypcje** - Toggle role notifications po zapełnieniu, ephemeral feedback
   - **Wiadomość o zapełnieniu lobby** (`messages.lobbyFull` + przycisk `toggle_party_notifications`): wysyłana w wątku **od razu po zapełnieniu**, przez `announceFullLobby` (z `handleFullLobby`) → `sendNotificationInvite`. Wcześniej czekała 30 s (`lobby.notificationInviteDelay` – ustawienie **usunięte**) i gracze mieli pełne lobby bez żadnej informacji
   - **Persistencja:** `notificationInviteAt` i `notificationInviteSent` w `lobbies.json`. Termin zapisywany jest **przed** wysyłką, więc nieudana wysyłka (wątek chwilowo niedostępny, restart w trakcie) zostaje w kolejce, a `restoreNotificationInvites` w `index.js` ponawia ją po starcie bota
   - **Wyrzucenie gracza przez właściciela** (`/party-kick`, gdy `isFull` spada na `false`) zdejmuje z kolejki wiadomość, której **nie udało się wysłać** (`cancelNotificationInvite`) - po ponownym zapełnieniu idzie od nowa. Wiadomości już wysłanej nie powtarzamy
