...
- **Pytanie o nagrodę:** 1 minuta po zapełnieniu lobby (`lobby.rewardPromptDelay`) bot wysyła w wątku pytanie z 13 przyciskami (same emoji nagród, bez opisów, 3 rzędy: 5+5+3)
- **Definicje nagród:** `config.rewards` - lista `{ key, name, emoji }` (Pet AW, RC, Chip, AW, Czerwona kolekcja, Mount Core, Chest Core Selector, Pet Crystal, Panda Shard, Transmute Core, Mount Shards Chest, Epic Tech Selector, Chest S Selector). Kolejność w tablicy = kolejność przycisków i kolumn w rankingu
...
- **Party przestało być pełne → pytanie znika** (`cancelRewardPrompt`): gdy ktoś wyjdzie z wątku (`ThreadMembersUpdate` w `index.js`) albo zostanie wykopany (`/party-kick`) i `isFull` spada na `false`, bot kasuje wiadomość z nagrodami i zeruje stan (`rewardPromptSent`, `rewardPromptAt`, `rewardPromptMessageId`, `rewardPromptMessagesSince`) - przepisywanie się zatrzymuje
  ...
  - `sendRewardPrompt` sprawdza przed wysłaniem `isFull`, obecność `rewardPromptAt` oraz czy zaplanowany termin już minął - dzięki temu `setTimeout` sprzed anulowania nie wystrzeli pytania przedwcześnie
